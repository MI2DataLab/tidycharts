...
library(ibcsplot)
# plotting_files <- 
#   file.path('ibcsplot/R',
#             list.files("ibcsplot/R"))
# sapply(c(plotting_files), source)
...
#barchart_plot_normalized(df, unique(p$island), c("Adelie", "Chinstrap", "Gentoo"),  c("Adelie", "Chinstrap", "Gentoo" )) %>% #SVGrenderer()
...
p <- penguins_raw %>%
  ...
  count() %>% 
  pivot_wider(names_from = species, values_from = n, values_fill = list(n = 0)) %>% as.data.frame()


line_plot_stacked(p, p$`Date Egg`, series = c('Adelie', 'Gentoo', 'Chinstrap'), series_labels = c('Adelie', 'Gentoo', 'Chinstrap'), show_labels = rep(NA, length.out = length(p$`Date Egg`))) %>% SVGrenderer()
```
