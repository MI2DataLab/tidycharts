...
p <- penguins_raw %>%
  ...
  group_by(week = week(`Date Egg`), year = year(`Date Egg`)) %>% 
...
line_plot_stacked(p, p$date, series = c('Adelie', 'Gentoo', 'Chinstrap'), series_labels = c('Adelie', 'Gentoo', 'Chinstrap'), show_labels = rep(NA, length.out = 14)) %>% SVGrenderer()
```
