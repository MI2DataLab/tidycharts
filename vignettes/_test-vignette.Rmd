...
## Column charts with units

```{r}
column_chart(df, x = df$Species, series = c("avg"), styles = c('actual', 'actual', 'forecast'), unit = 20) %>% SVGrenderer()
column_chart(df, x = df$Species, series = c("avg"), unit = 20) %>%
    add_title(line1 = 'Iris', line2_measure = "Typical measure", line2_rest = "in cm", line3 = "2020..2021") %>% SVGrenderer()
...
column_chart_waterfall(df, x = df$Species, series = c("avg"), unit = 10) %>% SVGrenderer()
...
column_chart_absolute_variance(df4$month, df4$s1, df4$s2, colors = 1, x_title = 'PL', x_style = 'plan', unit = 50) %>% SVGrenderer()
